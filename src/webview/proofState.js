...
(() => {
  ...
  var Fragment = class _Fragment {
    ...
    */
    append(other) {
      ...
      if (!this.size)
        return other;
      let last = this.lastChild, first3 = other.firstChild, content = this.content.slice(), i = 0;
      if (last.isText && last.sameMarkup(first3)) {
        content[content.length - 1] = last.withText(last.text + first3.text);
        i = 1;
      }
    ...
    findFittable() {
      ...
      for (let pass = 1; pass <= 2; pass++) {
        for (let sliceDepth = pass == 1 ? startDepth : this.unplaced.openStart; sliceDepth >= 0; sliceDepth--) {
          ...
          let first3 = fragment.firstChild;
          for (let frontierDepth = this.depth; frontierDepth >= 0; frontierDepth--) {
            ...
            else if (pass == 2 && first3 && (wrap2 = match.findWrapping(first3.type)))
  ...
  function closeFragment(fragment, depth, oldOpen, newOpen, parent) {
    if (depth < oldOpen) {
      let first3 = fragment.firstChild;
      fragment = fragment.replaceChild(0, first3.copy(closeFragment(first3.content, depth + 1, oldOpen, newOpen, first3)));
    }
    if (depth > newOpen) {
  ...
  handlers.copy = editHandlers.cut = (view, _event) => {
    let event = _event;
    let sel = view.state.selection, cut3 = event.type == "cut";
    if (sel.empty)
      return;
  ...
  function splitBlockAs(splitNode) {
    return (state, dispatch) => {
      ...
      if (!atEnd && atStart && $from.node(splitDepth).type != deflt) {
        let first3 = tr2.mapping.map($from.before(splitDepth)), $first = tr2.doc.resolve(first3);
  ...
  function doWrapInList(tr2, range, wrappers, joinBefore, listType) {
    ...
    for (let i = range.startIndex, e = range.endIndex, first3 = true; i < e; i++, first3 = false) {
      if (!first3 && canSplit(tr2.doc, splitPos, splitDepth)) {
  ...
  var Extension = class _Extension2 {
    constructor(config = {}) {
      this.type = "extension";
      this.name = "extension";
      this.parent = null;
    ...
    extend(extendedConfig = {}) {
      const extension = new _Extension2({ ...this.config, ...extendedConfig });
      extension.parent = this;
      this.child = extension;
      extension.name = extendedConfig.name ? extendedConfig.name : extension.parent.name;
      if (extendedConfig.defaultOptions && Object.keys(extendedConfig.defaultOptions).length > 0) {
  ...
  var setContent = (content, emitUpdate = false, parseOptions = {}, options = {}) => ({ editor: editor2, tr: tr2, dispatch, commands: commands2 }) => {
    var _a, _b;
    const { doc: doc3 } = tr2;
    ...
    if (dispatch) {
      if (can) {
        ...
        if (deflt && !atEnd && !$from.parentOffset && $from.parent.type !== deflt) {
          const first3 = tr2.mapping.map($from.before());
          const $first = tr2.doc.resolve(first3);
  ...
  var toggleList = (listTypeOrName, itemTypeOrName, keepMarks, attributes = {}) => ({ editor: editor2, tr: tr2, state, dispatch, chain, commands: commands2, can }) => {
...
.tippy-box[data-animation=fade][data-state=hidden] {
  ...
  var CommandManager2 = class {
    ...
    get commands() {
      ...
      const props = this.buildProps(tr2);
      return Object.fromEntries(
        Object.entries(rawCommands).map(([name, command22]) => {
          const method = (...args) => {
            const callback = command22(...args)(props);
            if (!tr2.getMeta("preventDispatch") && !this.hasCustomState) {
              view.dispatch(tr2);
            }
            return callback;
    ...
    createCan(startTr) {
      ...
      const formattedCommands = Object.fromEntries(
        Object.entries(rawCommands).map(([name, command22]) => {
          return [name, (...args) => command22(...args)({ ...props, dispatch: void 0 })];
        })
      );
      return {
        ...formattedCommands,
    ...
    buildProps(tr2, shouldDispatch = true) {
      ...
      const props = {
        ...
        get commands() {
          return Object.fromEntries(
            Object.entries(rawCommands).map(([name, command22]) => {
              return [name, (...args) => command22(...args)(props)];
            })
          );
        }
      };
  ...
  var clearContent2 = (emitUpdate = true) => ({ commands: commands2 }) => {
    return commands2.setContent("", { emitUpdate });
  };
  var clearNodes2 = () => ({ state, tr: tr2, dispatch }) => {
    const { selection } = tr2;
    const { ranges } = selection;
    if (!dispatch) {
      return true;
  ...
  var deleteCurrentNode2 = () => ({ tr: tr2, dispatch }) => {
    const { selection } = tr2;
    const currentNode = selection.$anchor.node();
    if (currentNode.content.size > 0) {
      return false;
  ...
  var deleteNode2 = (typeOrName) => ({ tr: tr2, state, dispatch }) => {
  ...
  var deleteRange3 = (range) => ({ tr: tr2, dispatch }) => {
    const { from: from2, to } = range;
    if (dispatch) {
      tr2.delete(from2, to);
    }
  ...
  var extendMarkRange2 = (typeOrName, attributes = {}) => ({ tr: tr2, state, dispatch }) => {
  ...
  var insertContent2 = (value, options) => ({ tr: tr2, commands: commands2 }) => {
    return commands2.insertContentAt({ from: tr2.selection.from, to: tr2.selection.to }, value, options);
  };
  ...
  var insertContentAt2 = (position, value, options) => ({ tr: tr2, dispatch, editor: editor2 }) => {
    ...
    if (dispatch) {
      ...
      if (isOnlyTextContent) {
        ...
        const isTextSelection22 = $fromNode.isText || $fromNode.isTextblock;
        const hasContent = $fromNode.content.size > 0;
        if (fromSelectionAtStart && isTextSelection22 && hasContent) {
          from2 = Math.max(0, from2 - 1);
        }
        tr2.replaceWith(from2, to, newContent);
      }
      if (options.updateSelection) {
        selectionToInsertionEnd3(tr2, tr2.steps.length - 1, -1);
      }
      if (options.applyInputRules) {
        tr2.setMeta("applyInputRules", { from: from2, text: newContent });
      }
  ...
  var joinItemBackward2 = () => ({ state, dispatch, tr: tr2 }) => {
    try {
      const point = joinPoint(state.doc, state.selection.$from.pos, -1);
      if (point === null || point === void 0) {
        return false;
  ...
  var joinItemForward2 = () => ({ state, dispatch, tr: tr2 }) => {
    try {
      const point = joinPoint(state.doc, state.selection.$from.pos, 1);
      if (point === null || point === void 0) {
        return false;
  ...
  var resetAttributes2 = (typeOrName, attributes) => ({ tr: tr2, state, dispatch }) => {
    let nodeType = null;
    let markType = null;
    const schemaType = getSchemaTypeNameByName2(
    ...
    if (dispatch) {
      tr2.selection.ranges.forEach((range) => {
        state.doc.nodesBetween(range.$from.pos, range.$to.pos, (node, pos) => {
          if (nodeType && nodeType === node.type) {
            tr2.setNodeMarkup(pos, void 0, deleteProps2(node.attrs, attributes));
          }
          if (markType && node.marks.length) {
            node.marks.forEach((mark) => {
              if (markType === mark.type) {
                tr2.addMark(pos, pos + node.nodeSize, markType.create(deleteProps2(mark.attrs, attributes)));
              }
            });
          }
        });
  ...
  var scrollIntoView2 = () => ({ tr: tr2, dispatch }) => {
    if (dispatch) {
      tr2.scrollIntoView();
    }
    return true;
  };
  var selectAll3 = () => ({ tr: tr2, dispatch }) => {
    if (dispatch) {
      const selection = new AllSelection(tr2.doc);
      tr2.setSelection(selection);
    }
  ...
  var setContent2 = (content, { errorOnInvalidContent, emitUpdate = true, parseOptions = {} } = {}) => ({ editor: editor2, tr: tr2, dispatch, commands: commands2 }) => {
    const { doc: doc3 } = tr2;
    if (parseOptions.preserveWhitespace !== "full") {
  ...
  function flattenExtensions(extensions) {
    return extensions.map((extension) => {
      ...
      const addExtensions = getExtensionField2(extension, "addExtensions", context);
      if (addExtensions) {
  ...
  function getHTMLFromFragment2(fragment, schema) {
    const documentFragment = DOMSerializer.fromSchema(schema).serializeFragment(fragment);
    const temporaryDocument = document.implementation.createHTMLDocument();
    const container = temporaryDocument.createElement("div");
    container.appendChild(documentFragment);
  ...
  function getAttributesFromExtensions2(extensions) {
    const extensionAttributes = [];
    const { nodeExtensions, markExtensions } = splitExtensions2(extensions);
    const nodeAndMarkExtensions = [...nodeExtensions, ...markExtensions];
    const defaultAttribute = {
      default: null,
      validate: void 0,
    ...
    extensions.forEach((extension) => {
      ...
      const addGlobalAttributes = getExtensionField2(
        extension,
        "addGlobalAttributes",
        context
      );
    ...
    nodeAndMarkExtensions.forEach((extension) => {
      ...
      const addAttributes = getExtensionField2(
        extension,
        "addAttributes",
        context
      );
  ...
  function injectExtensionAttributesToParseRule2(parseRule, extensionAttributes) {
    if ("style" in parseRule) {
      return parseRule;
    }
    return {
      ...
      getAttrs: (node) => {
        ...
        const newAttributes = extensionAttributes.reduce((items, item) => {
          const value = item.attribute.parseHTML ? item.attribute.parseHTML(node) : fromString2(node.getAttribute(item.name));
          if (value === null || value === void 0) {
            return items;
          }
          return {
  ...
  function cleanUpSchemaItem2(data) {
    return Object.fromEntries(
      // @ts-ignore
      Object.entries(data).filter(([key, value]) => {
        if (key === "attrs" && isEmptyObject2(value)) {
          return false;
        }
        return value !== null && value !== void 0;
      })
  ...
  function getSchemaByResolvedExtensions2(extensions, editor2) {
    var _a;
    const allAttributes = getAttributesFromExtensions2(extensions);
    const { nodeExtensions, markExtensions } = splitExtensions2(extensions);
    const topNode = (_a = nodeExtensions.find((extension) => getExtensionField2(extension, "topNode"))) == null ? void 0 : _a.name;
    const nodes = Object.fromEntries(
      nodeExtensions.map((extension) => {
        ...
        const extraNodeFields = extensions.reduce((fields, e) => {
          const extendNodeSchema = getExtensionField2(e, "extendNodeSchema", context);
          return {
            ...fields,
            ...extendNodeSchema ? extendNodeSchema(extension) : {}
          };
        }, {});
        const schema = cleanUpSchemaItem2({
          ...extraNodeFields,
          content: callOrReturn2(getExtensionField2(extension, "content", context)),
          marks: callOrReturn2(getExtensionField2(extension, "marks", context)),
          group: callOrReturn2(getExtensionField2(extension, "group", context)),
          inline: callOrReturn2(getExtensionField2(extension, "inline", context)),
          atom: callOrReturn2(getExtensionField2(extension, "atom", context)),
          selectable: callOrReturn2(getExtensionField2(extension, "selectable", context)),
          draggable: callOrReturn2(getExtensionField2(extension, "draggable", context)),
          code: callOrReturn2(getExtensionField2(extension, "code", context)),
          whitespace: callOrReturn2(getExtensionField2(extension, "whitespace", context)),
          linebreakReplacement: callOrReturn2(
            getExtensionField2(extension, "linebreakReplacement", context)
          ),
          defining: callOrReturn2(getExtensionField2(extension, "defining", context)),
          isolating: callOrReturn2(getExtensionField2(extension, "isolating", context)),
        ...
        const parseHTML = callOrReturn2(getExtensionField2(extension, "parseHTML", context));
        if (parseHTML) {
          schema.parseDOM = parseHTML.map(
            (parseRule) => injectExtensionAttributesToParseRule2(parseRule, extensionAttributes)
          );
        }
        const renderHTML = getExtensionField2(extension, "renderHTML", context);
        if (renderHTML) {
          schema.toDOM = (node) => renderHTML({
            node,
        ...
        const renderText = getExtensionField2(extension, "renderText", context);
        if (renderText) {
          schema.toText = renderText;
        }
        return [extension.name, schema];
    ...
    const marks = Object.fromEntries(
      markExtensions.map((extension) => {
        ...
        const extraMarkFields = extensions.reduce((fields, e) => {
          const extendMarkSchema = getExtensionField2(e, "extendMarkSchema", context);
          return {
            ...fields,
            ...extendMarkSchema ? extendMarkSchema(extension) : {}
          };
        ...
        const schema = cleanUpSchemaItem2({
          ...extraMarkFields,
          inclusive: callOrReturn2(getExtensionField2(extension, "inclusive", context)),
          excludes: callOrReturn2(getExtensionField2(extension, "excludes", context)),
          group: callOrReturn2(getExtensionField2(extension, "group", context)),
          spanning: callOrReturn2(getExtensionField2(extension, "spanning", context)),
          code: callOrReturn2(getExtensionField2(extension, "code", context)),
          attrs: Object.fromEntries(
            extensionAttributes.map((extensionAttribute) => {
              var _a2, _b;
  ...
  function findDuplicates2(items) {
    const filtered = items.filter((el, index) => items.indexOf(el) !== index);
    return Array.from(new Set(filtered));
  }
  function sortExtensions(extensions) {
    const defaultPriority = 100;
    return extensions.sort((a, b) => {
      const priorityA = getExtensionField2(a, "priority") || defaultPriority;
      const priorityB = getExtensionField2(b, "priority") || defaultPriority;
      if (priorityA > priorityB) {
        return -1;
      }
      if (priorityA < priorityB) {
  ...
  function resolveExtensions(extensions) {
    ...
    const duplicatedNames = findDuplicates2(resolvedExtensions.map((extension) => extension.name));
    if (duplicatedNames.length) {
      console.warn(
  ...
  function getNodeAttributes2(state, typeOrName) {
    const type = getNodeType2(typeOrName, state.schema);
    const { from: from2, to } = state.selection;
    const nodes = [];
    state.doc.nodesBetween(from2, to, (node2) => {
  ...
  function getAttributes2(state, typeOrName) {
    const schemaType = getSchemaTypeNameByName2(
      typeof typeOrName === "string" ? typeOrName : typeOrName.name,
      state.schema
    );
    if (schemaType === "node") {
      return getNodeAttributes2(state, typeOrName);
    }
    if (schemaType === "mark") {
      return getMarkAttributes2(state, typeOrName);
    }
    return {};
  }
  ...
  function getSchemaTypeByName2(name, schema) {
    return schema.nodes[name] || schema.marks[name] || null;
  }
  ...
  var getTextContentFromNodes2 = ($from, maxMatch = 500) => {
    let textBefore = "";
    const sliceEndPos = $from.parentOffset;
    $from.parent.nodesBetween(Math.max(0, sliceEndPos - maxMatch), sliceEndPos, (node, pos, parent, index) => {
      var _a, _b;
  ...
  function isExtensionRulesEnabled2(extension, enabled) {
    if (Array.isArray(enabled)) {
      return enabled.some((enabledExtension) => {
        const name = typeof enabledExtension === "string" ? enabledExtension : enabledExtension.name;
        return name === extension.name;
  ...
  var setMark2 = (typeOrName, attributes = {}) => ({ tr: tr2, state, dispatch }) => {
    const { selection } = tr2;
    const { empty: empty2, ranges } = selection;
    const type = getMarkType2(typeOrName, state.schema);
    if (dispatch) {
    ...
    return canSetMark2(state, tr2, type);
  };
  var setMeta2 = (key, value) => ({ tr: tr2 }) => {
    tr2.setMeta(key, value);
    return true;
  };
  ...
  var setNodeSelection2 = (position) => ({ tr: tr2, dispatch }) => {
    if (dispatch) {
      const { doc: doc3 } = tr2;
  ...
  var setTextSelection2 = (position) => ({ tr: tr2, dispatch }) => {
    if (dispatch) {
      const { doc: doc3 } = tr2;
      const { from: from2, to } = typeof position === "number" ? { from: position, to: position } : position;
      const minPos = TextSelection.atStart(doc3).from;
  ...
  var splitBlock3 = ({ keepMarks = true } = {}) => ({ tr: tr2, state, dispatch, editor: editor2 }) => {
    ...
    if (dispatch) {
      if (can) {
        ...
        tr2.split(tr2.mapping.map($from.pos), 1, types);
        if (deflt && !atEnd && !$from.parentOffset && $from.parent.type !== deflt) {
          const first22 = tr2.mapping.map($from.before());
          const $first = tr2.doc.resolve(first22);
          if ($from.node(-1).canReplaceWith($first.index(), $first.index() + 1, deflt)) {
            tr2.setNodeMarkup(tr2.mapping.map($from.before()), deflt);
          }
        }
  ...
  var joinListBackwards2 = (tr2, listType) => {
    const list = findParentNode2((node) => node.type === listType)(tr2.selection);
    if (!list) {
      return true;
    }
    const before = tr2.doc.resolve(Math.max(0, list.pos - 1)).before(list.depth);
  ...
  var joinListForwards2 = (tr2, listType) => {
    const list = findParentNode2((node) => node.type === listType)(tr2.selection);
    if (!list) {
      return true;
    }
  ...
  var toggleList2 = (listTypeOrName, itemTypeOrName, keepMarks, attributes = {}) => ({ editor: editor2, tr: tr2, state, dispatch, chain, commands: commands2, can }) => {
    const { extensions, splittableMarks } = editor2.extensionManager;
    ...
    if (range.depth >= 1 && parentList && range.depth - parentList.depth <= 1) {
      ...
      if (isList2(parentList.node.type.name, extensions) && listType.validContent(parentList.node.content) && dispatch) {
        return chain().command(() => {
          tr2.setNodeMarkup(parentList.pos, listType);
          return true;
        }).command(() => joinListBackwards2(tr2, listType)).command(() => joinListForwards2(tr2, listType)).run();
      }
    }
    if (!keepMarks || !marks || !dispatch) {
      return chain().command(() => {
        ...
        return commands2.clearNodes();
      }).wrapInList(listType, attributes).command(() => joinListBackwards2(tr2, listType)).command(() => joinListForwards2(tr2, listType)).run();
    }
    return chain().command(() => {
      const canWrapInList = can().wrapInList(listType, attributes);
      const filteredMarks = marks.filter((mark) => splittableMarks.includes(mark.type.name));
  ...
  var toggleMark2 = (typeOrName, attributes = {}, options = {}) => ({ state, commands: commands2 }) => {
    const { extendEmptyMarkRange = false } = options;
    const type = getMarkType2(typeOrName, state.schema);
  ...
  var toggleNode2 = (typeOrName, toggleTypeOrName, attributes = {}) => ({ state, commands: commands2 }) => {
    const type = getNodeType2(typeOrName, state.schema);
  ...
  var toggleWrap2 = (typeOrName, attributes = {}) => ({ state, commands: commands2 }) => {
    const type = getNodeType2(typeOrName, state.schema);
    const isActive2 = isNodeActive2(state, type, attributes);
    if (isActive2) {
      return commands2.lift(type);
  ...
  var unsetAllMarks2 = () => ({ tr: tr2, dispatch }) => {
    const { selection } = tr2;
    const { empty: empty2, ranges } = selection;
    if (empty2) {
      return true;
  ...
  var unsetMark2 = (typeOrName, options = {}) => ({ tr: tr2, state, dispatch }) => {
    var _a;
    const { extendEmptyMarkRange = false } = options;
    const { selection } = tr2;
    const type = getMarkType2(typeOrName, state.schema);
  ...
  var updateAttributes2 = (typeOrName, attributes = {}) => ({ tr: tr2, state, dispatch }) => {
    let nodeType = null;
    let markType = null;
    const schemaType = getSchemaTypeNameByName2(
      typeof typeOrName === "string" ? typeOrName : typeOrName.name,
  ...
  var inputRuleMatcherHandler2 = (text, find2) => {
    if (isRegExp2(find2)) {
      return find2.exec(text);
    }
    const inputRuleMatch = find2(text);
  ...
  function run2(config) {
    ...
    let matched = false;
    const textBefore = getTextContentFromNodes2($from) + text;
    rules.forEach((rule) => {
      if (matched) {
        return;
      }
      const match = inputRuleMatcherHandler2(textBefore, rule.find);
      if (!match) {
        return;
      }
      const tr2 = view.state.tr;
      ...
      const { commands: commands2, chain, can } = new CommandManager2({
        editor: editor2,
        state
      });
      const handler = rule.handler({
  ...
  function inputRulesPlugin2(props) {
    const { editor: editor2, rules } = props;
    const plugin = new Plugin({
      state: {
        init() {
          ...
          if (isSimulatedInput) {
            setTimeout(() => {
              ...
              if (typeof text === "string") {
                ...
                text = getHTMLFromFragment2(Fragment.from(text), state.schema);
              }
              const { from: from2 } = simulatedInputMeta;
              const to = from2 + text.length;
              run2({
  ...
  var Extendable = class {
    ...
    get options() {
      return {
        ...callOrReturn2(
          getExtensionField2(this, "addOptions", {
            name: this.name
          })
        ) || {}
      };
    ...
    get storage() {
      return {
        ...callOrReturn2(
          getExtensionField2(this, "addStorage", {
            name: this.name,
            options: this.options
          })
        ) || {}
  ...
  var Mark3 = class _Mark extends Extendable {
    constructor() {
      super(...arguments);
      this.type = "mark";
    }
  ...
  var pasteRuleMatcherHandler2 = (text, find2, event) => {
    if (isRegExp2(find2)) {
      return [...text.matchAll(find2)];
    }
    const matches2 = find2(text, event);
  ...
  function run22(config) {
    ...
    state.doc.nodesBetween(from2, to, (node, pos) => {
      ...
      const matches2 = pasteRuleMatcherHandler2(textToMatch, rule.find, pasteEvent);
      matches2.forEach((match) => {
        if (match.index === void 0) {
          return;
        }
  ...
  var tiptapDragFromOtherEditor2 = null;
  var createClipboardPasteEvent2 = (text) => {
    var _a;
    const event = new ClipboardEvent("paste", {
      clipboardData: new DataTransfer()
    });
  ...
  function pasteRulesPlugin2(props) {
    const { editor: editor2, rules } = props;
    let dragSourceElement = null;
    let isPastedFromProseMirror = false;
    let isDroppedFromProseMirror = false;
    ...
    const processEvent = ({
      ...
      const tr2 = state.tr;
      const chainableState = createChainableState2({
        state,
        transaction: tr2
      });
      const handler = run22({
      ...
      return new Plugin({
        ...
        view(view) {
          const handleDragstart = (event) => {
            ...
            if (dragSourceElement) {
              tiptapDragFromOtherEditor2 = editor2;
            }
          };
          const handleDragend = () => {
            if (tiptapDragFromOtherEditor2) {
              tiptapDragFromOtherEditor2 = null;
            }
          };
          window.addEventListener("dragstart", handleDragstart);
          window.addEventListener("dragend", handleDragend);
        ...
        props: {
          handleDOMEvents: {
            drop: (view, event) => {
              ...
              dropEvent = event;
              if (!isDroppedFromProseMirror) {
                const dragFromOtherEditor = tiptapDragFromOtherEditor2;
        ...
        appendTransaction: (transactions, oldState, state) => {
          ...
          if (isSimulatedPaste) {
            ...
            const pasteEvt = createClipboardPasteEvent2(text);
            return processEvent({
              rule,
              state,
              from: from22,
  ...
  var ExtensionManager2 = class {
    constructor(extensions, editor2) {
      this.splittableMarks = [];
      this.editor = editor2;
      this.baseExtensions = extensions;
      this.extensions = resolveExtensions(extensions);
      this.schema = getSchemaByResolvedExtensions2(this.extensions, editor2);
      this.setupExtensions();
    }
    /**
     * Get all commands from the extensions.
    ...
    get commands() {
      return this.extensions.reduce((commands2, extension) => {
        const context = {
          ...
          editor: this.editor,
          type: getSchemaTypeByName2(extension.name, this.schema)
        };
        const addCommands = getExtensionField2(extension, "addCommands", context);
        if (!addCommands) {
          return commands2;
        }
        return {
    ...
    get plugins() {
      ...
      const allPlugins = extensions.flatMap((extension) => {
        ...
        const plugins = [];
        const addKeyboardShortcuts = getExtensionField2(
          extension,
          "addKeyboardShortcuts",
          context
        );
        let defaultBindings = {};
        if (extension.type === "mark" && getExtensionField2(extension, "exitable", context)) {
          defaultBindings.ArrowRight = () => Mark3.handleExit({ editor: editor2, mark: extension });
        }
        if (addKeyboardShortcuts) {
        ...
        const keyMapPlugin = keymap(defaultBindings);
        plugins.push(keyMapPlugin);
        const addInputRules = getExtensionField2(extension, "addInputRules", context);
        if (isExtensionRulesEnabled2(extension, editor2.options.enableInputRules) && addInputRules) {
          const rules = addInputRules();
          if (rules && rules.length) {
            const inputResult = inputRulesPlugin2({
              editor: editor2,
              rules
            });
        ...
        const addPasteRules = getExtensionField2(extension, "addPasteRules", context);
        if (isExtensionRulesEnabled2(extension, editor2.options.enablePasteRules) && addPasteRules) {
          const rules = addPasteRules();
          if (rules && rules.length) {
            const pasteRules = pasteRulesPlugin2({ editor: editor2, rules });
            plugins.push(...pasteRules);
          }
        }
        const addProseMirrorPlugins = getExtensionField2(
          extension,
          "addProseMirrorPlugins",
          context
        );
     ...
     */
    get attributes() {
      return getAttributesFromExtensions2(this.extensions);
    }
    /**
     * Get all node views from the extensions.
     * @returns An object with all node views where the key is the node name and the value is the node view function
    ...
    get nodeViews() {
      const { editor: editor2 } = this;
      const { nodeExtensions } = splitExtensions2(this.extensions);
      return Object.fromEntries(
        nodeExtensions.filter((extension) => !!getExtensionField2(extension, "addNodeView")).map((extension) => {
          const extensionAttributes = this.attributes.filter((attribute) => attribute.type === extension.name);
          const context = {
            name: extension.name,
            options: extension.options,
          ...
          const addNodeView = getExtensionField2(extension, "addNodeView", context);
          if (!addNodeView) {
            return [];
          }
    ...
    get markViews() {
      const { editor: editor2 } = this;
      const { markExtensions } = splitExtensions2(this.extensions);
      return Object.fromEntries(
        markExtensions.filter((extension) => !!getExtensionField2(extension, "addMarkView")).map((extension) => {
          const extensionAttributes = this.attributes.filter((attribute) => attribute.type === extension.name);
          const context = {
            name: extension.name,
            options: extension.options,
            storage: this.editor.extensionStorage[extension.name],
            editor: editor2,
            type: getMarkType2(extension.name, this.schema)
          };
          const addMarkView = getExtensionField2(extension, "addMarkView", context);
          if (!addMarkView) {
            return [];
          }
    ...
    setupExtensions() {
      ...
      extensions.forEach((extension) => {
        ...
        if (extension.type === "mark") {
          const keepOnSplit = (_a = callOrReturn2(getExtensionField2(extension, "keepOnSplit", context))) != null ? _a : true;
          if (keepOnSplit) {
            this.splittableMarks.push(extension.name);
          }
        }
        const onBeforeCreate = getExtensionField2(extension, "onBeforeCreate", context);
        const onCreate = getExtensionField2(extension, "onCreate", context);
        const onUpdate = getExtensionField2(extension, "onUpdate", context);
        const onSelectionUpdate = getExtensionField2(
          extension,
          "onSelectionUpdate",
          context
        );
        const onTransaction = getExtensionField2(extension, "onTransaction", context);
        const onFocus = getExtensionField2(extension, "onFocus", context);
        const onBlur = getExtensionField2(extension, "onBlur", context);
        const onDestroy = getExtensionField2(extension, "onDestroy", context);
        if (onBeforeCreate) {
          this.editor.on("beforeCreate", onBeforeCreate);
        }
        if (onCreate) {
  ...
  ExtensionManager2.resolve = resolveExtensions;
  ExtensionManager2.sort = sortExtensions;
  ExtensionManager2.flatten = flattenExtensions;
...
`;
  ...
  var Blockquote = Node3.create({
    ...
    renderHTML({ HTMLAttributes }) {
      return /* @__PURE__ */ h("blockquote", { ...mergeAttributes2(this.options.HTMLAttributes, HTMLAttributes), children: /* @__PURE__ */ h("slot", {}) });
    },
...
`),
  ...
  function stringToArray(str) {
    ...
    let index = 0;
    while (index < len) {
      let first3 = str.charCodeAt(index);
      let second;
      let char = first3 < 55296 || first3 > 56319 || index + 1 === len || (second = str.charCodeAt(index + 1)) < 56320 || second > 57343 ? str[index] : str.slice(index, index + 2);
      result.push(char);
      index += char.length;
    }
    return result;
...
3. "-" cannot repeat`);
        ...
        changes.forEach(({ newRange }) => {
          ...
          if (textBlock && textBeforeWhitespace) {
            ...
            linksBeforeSpace.filter((link) => link.isLink).map((link) => ({
              ...
              if (getMarksBetween2(link.from, link.to, newState.doc).some((item) => item.mark.type === options.type)) {
                return;
              }
              tr2.addMark(
                link.from,
    ...
    return new Plugin({
      ...
      props: {
        handleClick: (view, pos, event) => {
          ...
          const attrs = getAttributes2(view.state, options.type.name);
  ...
  // node_modules/@tiptap/starter-kit/node_modules/@tiptap/extension-list/dist/index.js
  ...
  var handleBackspace = (editor2, name, parentListTypes) => {
    ...
    if (!isNodeActive2(editor2.state, name) && hasListBefore(editor2.state, name, parentListTypes)) {
    ...
    if (!isNodeActive2(editor2.state, name)) {
      return false;
    }
...
${nextLine.slice(indentLevel + 2)}`;
  ...
  var Underline = Mark3.create({
    ...
    renderHTML({ HTMLAttributes }) {
      return ["u", mergeAttributes2(this.options.HTMLAttributes, HTMLAttributes), 0];
    },
    parseMarkdown(token, helpers) {
  ...
  // node_modules/@tiptap/starter-kit/node_modules/@tiptap/extensions/dist/index.js
  var CharacterCount = Extension2.create({
    name: "characterCount",
    addOptions() {
      return {
        limit: null,
  ...
  var Gapcursor = Extension2.create({
    ...
    extendNodeSchema(extension) {
      ...
      return {
        allowGapCursor: (_a = callOrReturn2(getExtensionField2(extension, "allowGapCursor", context))) != null ? _a : null
      };
    }
  });
  var Placeholder = Extension2.create({
    name: "placeholder",
    addOptions() {
      return {
          ...
          props: {
            decorations: ({ doc: doc3, selection }) => {
              ...
              doc3.descendants((node, pos) => {
                ...
                const isEmpty = !node.isLeaf && isNodeEmpty2(node);
  ...
  var Selection2 = Extension2.create({
    name: "selection",
    addOptions() {
      return {
        className: "selection"
  ...
  var UndoRedo = Extension2.create({
    name: "undoRedo",
    addOptions() {
      return {
        depth: 100,
  ...
  // node_modules/track-change-extension/dist/index.js
  var MARK_DELETION = "deletion";
  var MARK_INSERTION = "insertion";
  var EXTENSION_NAME = "trackchange";
  var TRACK_COMMAND_ACCEPT = "accept";
  var TRACK_COMMAND_ACCEPT_ALL = "accept-all";
  var TRACK_COMMAND_REJECT = "reject";
  var TRACK_COMMAND_REJECT_ALL = "reject-all";
  var InsertionMark = Mark2.create({
    name: MARK_INSERTION,
    addAttributes() {
      return {
        "data-op-user-id": {
  ...
  var IME_STATUS_NORMAL = 0;
  var IME_STATUS_CONTINUE = 2;
  var composingStatus = 0;
  var isStartChineseInput = false;
  var getSelfExt = (editor2) => editor2.extensionManager.extensions.find((item) => item.type === "extension" && item.name === EXTENSION_NAME);
  var getMinuteTime = () => Math.round((/* @__PURE__ */ new Date()).getTime() / 1e3 / 60) * 1e3 * 60;
  var changeTrack = (opType, param) => {
    const from2 = param.editor.state.selection.from;
    const to = param.editor.state.selection.to;
    let markRanges = [];
    if ((opType === TRACK_COMMAND_ACCEPT || opType === TRACK_COMMAND_REJECT) && from2 === to) {
      ...
      markRanges = getMarksBetween(from2, to, param.editor.state.doc);
    }
  ...
  var TrackChangeExtension = Extension.create({
    name: EXTENSION_NAME,
    onCreate() {
    ...
    addCommands: () => {
      return {
        setTrackChangeStatus: (enabled) => (param) => {
        ...
        updateOpUserOption: (opUserId, opUserNickname) => (param) => {
          const thisExtension = getSelfExt(param.editor);
    ...
    // @ts-ignore
    onSelectionUpdate(p) {
      console.log("selection and input status", p.transaction.selection.from, p.transaction.selection.to, p.editor.view.composing);
    },
    // @ts-ignore
    addProseMirrorPlugins(props) {
      return [
        new Plugin({
          ...
          props: {
            handleDOMEvents: {
              ...
              compositionupdate: (_event) => {
                console.log("chinese input continue");
    ...
    // @ts-ignore
    onTransaction: (props) => {
      const { transaction, editor: editor2 } = props;
      ...
      const isThisTrApplied = transaction.before !== editor2.state.tr.doc;
      const thisExtension = getSelfExt(editor2);
      const trackChangeEnabled = thisExtension.options.enabled;
      console.warn("\u5185\u5BB9\u53D8\u5316\uFF0C\u6267\u884C\u8DDF\u8E2A\u4FEE\u8BA2\u76F8\u5173\u903B\u8F91", transaction.steps.length, transaction);
      const allSteps = transaction.steps.map((step) => Step.fromJSON(editor2.state.doc.type.schema, step.toJSON()));
      ...
      allSteps.forEach((step, _index, _arr) => {
        if (step instanceof ReplaceStep) {
          ...
          if (step.from !== step.to) {
            const slice2 = transaction.docs[_index].slice(step.from, step.to);
            slice2.content.forEach((node) => {
      ...
      const newChangeTr = isThisTrApplied ? editor2.state.tr : transaction;
      let reAddOffset = 0;
      allSteps.forEach((step, index) => {
        if (step instanceof ReplaceStep) {
          const invertedStep = step.invert(transaction.docs[index]);
          if (step.slice.size) {
            const insertionMark = editor2.state.doc.type.schema.marks.insertion.create({
            ...
            const deletionMark = editor2.state.doc.type.schema.marks.deletion.create();
            const from2 = step.from + reAddOffset;
            const to = step.from + reAddOffset + step.slice.size;
            if (trackChangeEnabled) {
              newChangeTr.addMark(from2, to, insertionMark);
            } else {
              newChangeTr.removeMark(from2, to, insertionMark.type);
            }
            newChangeTr.removeMark(from2, to, deletionMark.type);
          }
          if (step.from !== step.to && trackChangeEnabled) {
            ...
            const travelContent = (content, parentOffset) => {
            ...
            const { from: from2 } = reAddStep;
            const to = from2 + reAddStep.slice.size;
            newChangeTr.addMark(from2, to, newChangeTr.doc.type.schema.marks.deletion.create({
              "data-op-user-id": thisExtension.options.dataOpUserId,
              "data-op-user-nickname": thisExtension.options.dataOpUserNickname,
              "data-op-date": getMinuteTime()
            }));
            skipSteps.forEach((step2) => {
              newChangeTr.step(step2);
            });
          }
          const newState = editor2.state.apply(newChangeTr);
          editor2.view.updateState(newState);
        }
      });
      const finalNewPos = trackChangeEnabled ? currentNewPos + posOffset : currentNewPos;
      if (trackChangeEnabled) {
        const trWithChange = editor2.view.state.tr;
        trWithChange.setSelection(TextSelection.create(editor2.view.state.doc, finalNewPos));
        const newStateWithNewSelection = editor2.view.state.apply(trWithChange);
        console.log("update cursor", finalNewPos);
        editor2.view.updateState(newStateWithNewSelection);
      }
      if (isChineseStart && hasAddAndDelete && trackChangeEnabled) {
        editor2.commands.deleteSelection();
        editor2.commands.blur();
        setTimeout(() => {
  ...
  var tacticInput = document.getElementById("tacticInput");
  ...
  function renderGoalsToHtml(goals) {
    ...
    for (let i = 0; i < goals.length; i++) {
      ...
      if (g.hyps && g.hyps.length > 0) {
        html += '<div class="hyps">';
        g.hyps.forEach((h2) => {
          const text = escapeHtml(h2.names.join(", ") + ": " + h2.ty);
  ...
  if (editorElement) {
    editor = new Editor({
      ...
      extensions: [
        index_default,
        // Basic Tiptap nodes (doc, paragraph, text)
